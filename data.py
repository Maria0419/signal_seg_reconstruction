...
class SignalDataset(Dataset):
    def __init__(self, data_path, data_type, noise=None, nfold=0):
        self.noise = noise
        self.nfold = nfold
    ...
    def __getitem__(self, idx):
        data = h5py.File(self.data_list[idx], 'r')
        label_file = self.label_list[idx]
        image_file = self.image_list[idx]

        self.rand = random.random()

        signal = self.load_signal(data)
        label = self.load_image(label_file)
    ...
    def load_image(self, filename):
        ...
        if self.noise is not None and self.rand > 0.5:
...
data = SignalDataset(data_path, data_type='test', nfold=1)
