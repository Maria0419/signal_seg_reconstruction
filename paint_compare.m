...
subplot(2,3,3); imshow(ddimg); title(sprintf('UNETDD Reconstruction MSE %.05f',loss_unetdd.MSE(rank)));
...
subplot(2,3,5); imshow(result_sig); title(sprintf('Sig UNET-DiceL: %.04f',loss_sunet4c.Loss(id)));
subplot(2,3,6); imshow(result_ddsig); title(sprintf('DDSig UNET-DiceL: %.04f', loss_unetdd.Dice(rank)));

disp(loss_unetdd.Order(rank));